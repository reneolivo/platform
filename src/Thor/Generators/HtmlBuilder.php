...
class HtmlBuilder extends \Illuminate\Html\HtmlBuilder
{
     ...
     * @var array
     */
    protected $voidElements = array('area', 'base', 'br', 'col', 'command', 'embed',
     ...
     * @return string
     */
    public function content($elementName, $content)
    {
        ...
        if(is_array($content)) {
            foreach($content as $i => $c) {
                //option
                if(($elementName == 'select') or ( $elementName == 'optgroup') or ( $elementName == 'datalist')) {
                    $str .= $this->option(array('value' => $i), $c) . "\n";
                    //li
                } elseif(($elementName == 'ul') or ( $elementName == 'ol')) {
                    $str .= $this->li(null, $c) . "\n";
                    //td
                } elseif($elementName == 'tr') {
                    $str .= $this->td(null, $c) . "\n";
                    //tr + td
                } elseif(($elementName == 'table') or ( $elementName == 'thead') or ( $elementName == 'tbody') or ( $elementName == 'tfoot')) {
                    if(is_array($c)) {
                        $str .= $this->trOpen() . "\n";
                        foreach($c as $j => $tdc) {
                    ...
                    //a
                } elseif($elementName == 'nav') {
                    $str .= $this->a(array('href' => $i), $c) . "\n";
                    //source
                } elseif(($elementName == 'audio') or ( $elementName == 'video')) {
     ...
     * @return type
     */
    public function element($elementName, $attr = array(), $content = null)
    {
        $elementName = strtolower($elementName);
        return $this->elementOpen($elementName, $attr) . $this->content($elementName, $content) . $this->elementClose($elementName);
    }

    public function elementOpen($elementName, $attr = array())
    {
        $elementName = strtolower($elementName);
        if(!in_array($elementName, $this->voidElements)) {
            return "<{$elementName}" . rtrim($this->attributes($attr)) . ">";
        } else {
            return "<{$elementName}" . $this->attributes($attr);
        }
    }

    public function elementClose($elementName)
    {
        $elementName = strtolower($elementName);
        if(in_array($elementName, $this->voidElements)) {
            return " />\n";
    ...
    public function attrPluck($str, $elementName = 'a|img', $attrname = 'href|src')
    {
        $elementName = strtolower($elementName);
        preg_match_all('/<' . $elementName . '[^>]+>/i', $str, $result);
        $elems = array();
    ...
    public function addLinks($str, $target_blank = false)
    {
        if($target_blank) {
    ...
    public function wrapWords($string, $words, $elementName = 'mark', $elementAttrs = array())
    {
        if(!is_array($words)) {
            $words = array($words);
        }
        ...
        foreach($words as $word) {
            $string = str_ireplace($word, $this->element($elementName, $elementAttrs, $word), $string);
        }
        return $string;
    }
    ...
    public function __call($name, $arguments)
    {
        ...
        if(preg_match('/Open$/i', $name)) {
            return call_user_func_array(array($this, 'elementOpen'), $arguments);
        } elseif(preg_match('/Close$/i', $name)) {
            return call_user_func_array(array($this, 'elementClose'), $arguments);
        } else {
            return call_user_func_array(array($this, 'element'), $arguments);
        }
    }
